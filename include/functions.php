 ...
 * @return string
 */
function service_block_addCatSelect($cats)
{
	$cat_sql = '(';
	if (is_array($cats)) {
 ...
 */
function serviceGetMyItemIds($permtype, $dirname)
{
	global $xoopsUser;
	static $permissions = [];
	...
	$serviceModule = $moduleHandler->getByDirname($dirname);
	$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
	$grouppermHandler = xoops_getHandler('groupperm');
	$itemIds = $grouppermHandler->getItemIds($permtype, $groups, $serviceModule->getVar('mid'));
	return $itemIds;
}

/**
 * Get the number of services from the sub categories of a category or sub topics of or topic
 * @param $mytree
 * @param $services
 * @param $entries
 * @param $cid
 * @return int
 */
function serviceNumbersOfEntries($mytree, $services, $entries, $cid)
{
    $count = 0;
    if(in_array($cid, $services)) {
        $child = $mytree->getAllChild($cid);
        foreach (array_keys($entries) as $i) {
            if ($entries[$i]->getVar('ser_id') == $cid){
                $count++;
            }
            foreach (array_keys($child) as $j) {
                if ($entries[$i]->getVar('ser_id') == $j){
                    $count++;
                }
            }
        }
...
function serviceMetaKeywords($content)
{
...
function serviceMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function service_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = service_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function service_Filter($url, $type = '') {
